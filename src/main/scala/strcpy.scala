 ...
 */
class WithStrcpy extends Config ((site, here, up) => {
  case BuildRoCC => Seq((p: Parameters) => {
     val sc = LazyModule.apply(new Strcpy(OpcodeSet.custom0)(p))
     sc
  })
})
 ...
 */
class Strcpy(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCC(opcodes) {
